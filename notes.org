#+TITLE: Στοχαστικές Αριθμητικές Μέθοδοι και Εφαρμογές
#+SUBTITLE: Διδάσκων: Σαμπάνης Σ.
...
+ Langevin Stochastic DEs: βλέπουμε τις στοχαστικές λύσεις σαν στοχαστικές διαφορικές εξισώσεις.
...
+ Μέσα από την θεωρία του Stochastic Approximation: χρησιμοποιεί ΔΕ σαν εργαλεία
...
+ Ito's formula (σημαντικό) διαχωρίζει το δυναμικό σύστημα τ.ω. να μπορούμε να αναγνωρίζουμε ποια
  είναι τα martingales. Βλέπω τις τάσεις του δυναμικού συστήματος.

* ΔΙΑΔΙΚΑΣΤΙΚΑ ΜΑΘΗΜΑΤΟΣ
+ θα γίνει εξέταση
+ βιβλιογραφία:
  + (Θεωρία Πιθανοτήτων) David Williams : Probability with martingales
  + (Στοχαστικές Διαδικασίες/Ανάλυση) Καραντζάς & Steven
...
Ονομάζουμε σ-άλγεβρα Borel (ή Borel σύνολα), συμβ. $\mathcal{B}({\mathbb{R^d})$ (στο $\mathbb{R^d},\: d=1,2,\dots$), την ελάχιστη
σ-άλγεβρα (παραγόμενη) που περιέχει όλα τα ανοικτά υποσύνολα του $\mathbb{R}^d$.\\
\end{orismos}

\begin{protasi}
Η σ-άλγεβρα Borel είναι η μικρότερη σ-άλγεβρα που περιέχει τα διαστήματα της μορφής
...
Έστω $\mathcal{F} μια σ-άλγεβρα. Το σύνολο $A\in \mathcal{F}$ λέγεται $\mathcal{F}$-μετρήσιμο ($\mathcal{F}$-measurable).
\end{orismos}
...
1. (coutable subadditivity). Για κάθε $A_1,A_2,\dots \in \mathcal{F}$ έχουμε $P\big( \bigcup_{i\geq 1} \big) \leq \sum_{i\geq 1} P(A_i)$.
2. (monotonicity). Για κάθε $A,B \in \mathcal{F}$ με $A\subset B$ έχουμε $P(A)\leq P(B)$.
3. (continuity). Έστω $A_1 \subset A_2 \subset \dots$ όπου $A_1, A_2,\dots \in \mathcal{F}$ μια αύξουσα ακολουθία ενδεχομένων, τότε $\lim_{n\rightarrow \infty}P(A_n) = P \big( \bigcup_{n=1}^\infty A_n \big)$
...
*Ιδιότητα* (πηγάζει από την 3) Έστω $A_1 \supset A_2 \supset \dots$ (contracting sequence of events), τότε ισχύει ότι:
...
Συνεπώς είναι προφανές ότι αυτή η συνάρτηση δεν είναι Riemann ολοκληρώσιμη.\\
Ωστόσο παρατηρώ ότι η $f$ είναι η $\mathbf{1}_{[0,1]\setminus \mathbb{Q}$. Ποιο είναι το μέτρο Lebesgue του $A=[0,1]\mathbb{Q}$; Γνωρίζουμε ότι οι ρητοί
...
Απλές συναρτήσεις (step functions) είναι συναρτήσεις της μορφής
...
Θα ορίσουμε το ολοκλήρωμα Lebesgue σε τρία βήματα.\\
...
$$ f(x) = \sum_{i=1}^n c_i \mathbf{1}_{A_i} \qquad \text{όπου } \: A_i \cap A_j = \emptyset \text{ και } \bigcup_i A_i = \Omega$$
τότε ορζίουμε το ολοκλήρωμα Lebesgue της $f$ ως:
$$ \int_\Omega f d\mu = \sum_{i=1}^\inftu c_i \mu(A_i) \in [0,+\infty]$$
με την σύμβαση ότι στο ολοκλήρωμα Lebesgeue $(0\cdot \infty = 0)$.\\
...
** Ιδιότητες
1. Το ολοκλήρωμα Lebesgue μιας μετρήσιμης συνάρτησης, όπου αυτό ορίζεται, είναι ένα στοιχείο του $[0,\infty]$.
2. Αν το μέτρο ενός έστω από τα $A_i$ είναι ίσο με άπειρο, τότε το ολοκλήρωμα Lebesgue παίρενει την τιμή $+\infty$ (για κάθε $c_i >0,\: i\geq 1$).
...
5. Αν για μια μετρήσιμη συνάρτηση $f$ υπάρχει το γενικευμένο ολ. Riemann
...
6. Μπορώ να έχω το γενικευμένο ολ. Riemann αλλά όχι το αντίστοιχο Lebesgue. (π.χ. $f(x) = \frac{sinx}{x}\mathbf{1}_{\{x\neq 0\}}$)

** Κύριες Ιδιότητες του ολοκληρώματος Lebesgue:

+ \textbf{(Γραμμικότητα - Linearity)}   $$\int_\mathbb{R} (c_1 f + c_2 g) d\mu = c_1 \int_\mathbb{R} fd\mu + c_2 \int_\mathbb{R} gd\mu$$.
+ \textbf{(Ξένα Σύνολα - Disjoint Sets)} Αν $A,B$ είναι ξένα μεταξύ τους σύνολα, τότε
  $$\int_{A\cup B} f d\mu = \int_A fd\mu + \int_B fd\mu$$
+ \textbf{(Μονοτονία - Comparison)} Αν $f(x) \leq g(x)$ για κάθε $x\in \mathbb{R}$, τότε
...
Το λήμμα Fatou μας λέει ότι μπορεί να έχω μια ακολουθία μετρήσιμων τ.μ. που να συκλίνει σε μια (μετρήσιμη) τ.μ. αλλά οι ροπές τους
(moments) να μην συγκλίνουν!!
\pagebreak

\begin{theorima}\textbf{Θεώρημα Κυριαρχημένης Σύγκλισης - (Lebesgue) Dominated Convergence Theorem (LDCT).} Έστω $\{f_n\}_{n\geq 1}$ μια ακολουθία ολοκληρώσιμων συναρτήσεων
...
Τώρα θα κάνουμε χρήση του FT. Έστω $h_n := 2g - |f_n - f|$, άρα η $\{h_n\}$ είναι μια μη-αρνητική ακολουθία μετρήσιμων συναρτήσεων, εφαρμόζω το λήμμα Fatou και
...
*Σημείωση:* Τα παραπάνω τρία θεωρήματα σύκγλισης (MCT, FL, LDCT)  ισχύουν σε σ-πεπερασμένους χώρος μέτρου $(\Omega, \mathcal{F}, \mu)$.\\
...
*Παρατήρηση*: Στο Θεώρημα Radon Nikodym αυστηρά δεν έχουμε ορίσει κάποια παράγωγο μέτρου σε σχέση με κάποιο άλλο μέτρο, και ο συμβολισμός της πυκνότητας
...
*Σύνδεση/εφαρμογή με τα χρηματοοικονομικά μαθηματικα*: Προσπαθούμε να βρούμε ισοδύναμα μέτρα πιθανότητας ως προς το "φυσικό" μέτρο πιθανότητας έτσι ώστε να δημιουργήσουμε στο νέο μέτρο martingales (δίκαια παιχνίδια). Υπό το
...
\item η ακολουθία συγκλίνει σε μια τυχαία μεταβλητή $X$ \emph{κατά πιθανότητα} (in probability) και γράφουμε $X_n \stackrel{\mathbb{P}}{\rightarrow} X$ αν
$$\lim_{n\rightarrow \infty}\mathbb{P}(|X_n - X| > \epsilon) = 0 \qquad \forall \epsilon >0$$
\item η ακολουθία συγκίνει σε μια τυχαία μεταβλητή $X$ \emph{κατά κατανομή} (in distribution) και γράφουμε $X_n \stackrel{d}{\rightarrow} X$ αν
