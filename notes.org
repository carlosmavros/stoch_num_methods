...
$$\text{"} dX_t = b(X_t)dt + \sigma (X_t) dW_t \text{"} \qquad \omega \in \Omega$
...
** Κίνηση \textlatin{Brown}  / Διαδικασία \textlatin{Wiener}
...
**Σημείωση:** Πολλές φορές ο ισοδύναμος ορισμός χρησιμοποιείται, όταν το 1. δίνεται ως εξής:
$W_t - W_s$ είναι ανεξάρτητη από $\mathcal{G} = \sigma(W_r, r\leq s)$ για όλα τα $t,s$ τ.ω. $0\leq s \leq t$.\\

*Σημείωση:* Χρησιμοποιείται ο όρος \textlatin{Wiener martingale} ως προς $\{F_t\}_{t\geq 0}$ αν $W:= \{W_t\}_{t\geq 0}$ είναι προσαρμοσμένη (\textlatin{adapted}) στην $\{F_t\}_{t\geq 0}$ και $W_t - W_s$ είναι
...
** Κύμανση και τετραγωνικά κύμανση
...
**Παρατήρηση:** Κανονικά, όπου γράωαμε $t_{j}$ θα έπρεπε να γράφουμε $t_j^n$ αλλά για συντομία παραλέιπουμε το $n$.

i\begin{theorima}
Έστω $\{ \Pi_n \}_{n\geq 1}$ μια ακολουθθία διαμερίσεων του $A:=[s,t]$ τ.ω. $\Pi_m \subset \Pi_n$, αν $m>n$ \textlatin{(refining sequence of partitions)} και $\lim_{n\rightarrow \infty} ||\Pi_n|| = 0 $. Τότε
$$ Q^W(\Pi_n) \rightarrow t-s \quad \text{στον } L^2 \: \text{, όταν το } n\rightarrow \infty, \quad \Big(  [W]_A = t-s \quad \text{στον } L^2 \Big)$
δηλ.
$$\lim_{n\rightarrow \infty} \mathbb{E}\big[ |Q^W(\Pi_n) - (t-s)|^2 \big] = 0$$
\end{theorima}
