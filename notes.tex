...
\newpage
\section{Μάθημα 1}
\label{sec:org3da8b61}

\subsection{Εισαγωγή}
\label{sec:org2cd93dc}
...
\section{Μάθημα 2}
\label{sec:org71bf11b}

\subsection{Στοιχεία Θεωρίας Πιθανοτήτων}
\label{sec:org15e524c}
...
\end{itemize}
\section{Μάθημα 5}
\label{sec:org5a6ad05}

\subsubsection{Θεωρήματα Σύγκλισης}
\label{sec:org00049f9}
...
\section{Μάθημα 8}
\label{sec:org41bb09e}
\pagebreak

\section{Μάθημα 9}
\label{sec:org10c9511}
...
i\begin{theorima}
Έστω \(\{ \Pi_n \}_{n\geq 1}\) μια ακολουθθία διαμερίσεων του \(A:=[s,t]\) τ.ω. \(\Pi_m \subset \Pi_n\), αν \(m>n\) \textlatin{(refining sequence of partitions)} και \$\(\lim\)\textsubscript{n\(\rightarrow\) \(\infty\)} ||\(\Pi\)\textsubscript{n}|| = 0 \$. Τότε
$$ Q^W(\Pi_n) \rightarrow t-s \quad \text{στον } L^2 \: \text{, όταν το } n\rightarrow \infty, \quad \Big(  [W]_A = t-s \quad \text{στον } L^2 \Big)$
δηλ.
$$\(\lim\)\textsubscript{n\(\rightarrow\) \(\infty\)} \mathbb{E}\big[ |Q^W(\Pi_n) - (t-s)|^2 \big] = 0\$\$
\end{theorima}

\begin{proof}
\begin{align*}
\mathbb{E}\big[ |Q^W(\Pi_n) - (t-s) |^2\big] &= \mathbb{E}\Big[ \Big| \sum_{i=0}^{n-1} |W_{t_{i+1}^n} - W_{t_i^n}|^2 - \sum_{i=0}^{n-1}(t_{i+1}^n - t_i^n) \Big|^2 \Big]\\
&= \mathbb{E}\Big[ \Big| \sum_{i=0}^{n-1} \big( |W_{t_{i+1}^n} - W_{t_i^n}|^2 - (t_{i+1}^n - t_i^n) \big) \Big|^2 \Big]\\
&= \mathbb{E}\Big[ \sum_{i=0}^{n-1} \big( |W_{t_{i+1}^n} -W_{t_i^n}| - (t_{i+1}^n - t_{i}^n)  \big)^2 \Big + 2\sum_{i>j}\big(|W_{t_{i+1}^n} - W_{t_i^n}|^2 - (t_{i+1}^n - t_{i}^n) \big) \big( |W_{t_{j+1}^n} - W_{t_j^n}|^2 - (t_{j+1}^n - t_j^n) \big) \Big]\\
&= \sum_{i=0}^{n-1} \mathbb{E}\Big[ \big( |W_{t_{i+1}^n} - W_{t_i^n}|^2 \cdot (t_{i+1}^n - t_{i}^n) \big)^2 \Big]\\
&\qquad  + 2\sum_{i>j}\underbrace{\mathbb{E}\Big[ \big(|W_{t_{i+1}^n} - W_{t_i^n}|^2 - (t_{i+1}^n - t_i^n) \big) \Big]}_{ = 0 } \cdot \underbrace{\mathbb{E}\Big[ \big(|W_{t_{j+1}^n} - W_{t_j^n}|^2 - (t_{j+1}^n - t_j^n) \big) \Big]}_{ = 0 }\\
&= \sum_{i=0}^{n-1} \mathbb{E}\Big[ \Big( \underbrace{\big( \frac{|W_{t_{i+1}^n} - W_{t_i^}| }{\sqrt{t_{i+1}^n - t_i^n}} \big)^2}_{:= Z^2} \cdot (t_{i+1}^n - t_i^n)  - (t_{i+1}^n - t_i^n) \Big)^2 \Big]\\
&\leq \sum_{i=0}^{n-1} ||\Pi_n|| (t_{i+1}^n - t_i) \mathbb{E}\Big[ (Z^2 - 1)^2 \Big]\\
&= ||\Pi_n|| \mathbb{E}[(Z^2-1)^2]]\cdot (t-s) \rightarrow 0 \quad \text{όταν } n\rightarrow \infty
\end{align*}
\end{proof}

\begin{protasi} Για κάθε ακολουθία διαμερίσεων $\{\Pi_n\}_{n\geq 1}$ του $A:=[s,t]$ τέτοιων ώστε $\sum_{n=1}^\infty ||\Pi_n|| < \infty$, ισχύει
$$ Q^W(\Pi_n) \rightarrow t-s \: \text{ σ.β. } \qquad \Big( [W]_A = t-s \: \text{ σ.β. }\Big)$$
δηλ.
$$ \mathbb{P}\Big( Q^W(\Pi_n) \rightarrow t-s \: \text{ όταν το  } n\rightarrow \infty \Big) = 1$$
\end{protasi}
